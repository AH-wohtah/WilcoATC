...
/// hors-ligne comme le reste. Mesuré sur un corpus de phraséologie ATC synthétisée avec trois
/// voix différentes :
///
...
/// c'est le meilleur modèle PRÉSENT qui est retenu (voir <see cref="Resolve"/>).
/// </summary>
public sealed class SpeechModelRepository
{
    ...
    /// avant le multilingue à taille égale.
    /// </summary>
    private static int Score(SpeechModel m)
    {
        ...
        if (m.Engine == SpeechEngine.NemoTransducer)
            return 1000 + (n.Contains("0.6b") ? 10 : 0);
