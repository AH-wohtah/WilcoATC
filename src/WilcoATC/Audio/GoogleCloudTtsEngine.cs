...
public sealed class GoogleCloudTtsEngine : ITtsEngine
{
    ...
    /// <summary>Environment variable holding the API key, unless the settings name another.</summary>
    public const string DefaultKeyEnvVar = "WILCOATC_GOOGLE_KEY";

    /// <summary>The name this variable had before the project was renamed. Read as a fallback.</summary>
    public const string LegacyKeyEnvVar = "FREQWATCH_GOOGLE_KEY";
    ...
    public async Task<TtsAudio> SynthesizeAsync(string text, CancellationToken ct = default)
    {
        ...
        string envVar = string.IsNullOrWhiteSpace(keyEnvVar) ? DefaultKeyEnvVar : keyEnvVar;
        string? key = Environment.GetEnvironmentVariable(envVar);

        // The variable used to be named after the project's former name. Someone who set it
        // back then should not have to notice a rename, so the old name is still read — but
        // only as a fallback, and only when the new one is empty.
        if (string.IsNullOrWhiteSpace(key) && envVar == DefaultKeyEnvVar)
            key = Environment.GetEnvironmentVariable(LegacyKeyEnvVar);
