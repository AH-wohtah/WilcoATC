...
public sealed class SherpaOnnxTtsEngine : ITtsEngine, IDisposable
{
    ...
    /// <summary>
    /// Charge le modèle d'une voix sans synthétiser, pour que la première phrase ne paie
    /// pas la demi-seconde de chargement. Silencieux en cas d'échec : ce n'est qu'une
    /// optimisation, la synthèse rechargera si besoin.
    /// </summary>
    public void Preload(TtsVoice requested)
    {
        try
        {
            var (defaultVoiceName, _, _) = _config();
            var voice = _voices.Resolve(requested.Name ?? defaultVoiceName);
            if (voice is not null) EnsureLoaded(voice);
        }
        catch { /* préchargement au mieux */ }
    }
