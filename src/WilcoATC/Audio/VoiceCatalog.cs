...
///
/// LANGUE : anglais uniquement (US + GB). Le contrôleur ne parle pas d'autre langue pour
/// l'instant — voir <see cref="Atc.LanguageResolver.Effective"/>.
///
/// On peut toujours déposer manuellement n'importe quel modèle sherpa-onnx dans le dossier
/// des voix : il sera détecté automatiquement, et sa langue déduite de son nom.
/// </summary>
public static class VoiceCatalog
{
    public static readonly IReadOnlyList<CatalogVoice> Voices = new[]
    {
        ...
        // Les voix françaises, allemandes, espagnoles et italiennes ont été RETIRÉES du
        // catalogue : l'ATC ne parle qu'anglais pour l'instant, et proposer une voix qu'il
        // n'emploiera jamais ne ferait que gaspiller la bande passante de l'utilisateur.
        // Elles se remettent en même temps que le multilingue (voir LanguageResolver).
    };

    /// <summary>Voix du catalogue pour une langue donnée (code ISO : « fr », « de »…).</summary>
    public static IReadOnlyList<CatalogVoice> ForLanguage(string languageCode)
        => Voices.Where(v => string.Equals(VoicePicker.LanguageOf(v.Name), languageCode,
