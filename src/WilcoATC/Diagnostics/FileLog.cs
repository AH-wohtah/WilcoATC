...
/// <summary>
/// Journal de diagnostic sur DISQUE, dans <c>%LOCALAPPDATA%\WilcoATC\logs</c>.
///
...
public static class FileLog
{
    ...
    public static string Directory => Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "WilcoATC", "logs");
    ...
    {
        // FIRST, BEFORE ANYTHING ELSE TOUCHES THE DATA FOLDERS: bring across what the former
        // name left behind. Opening the log would itself create the new folder, which would
        // make the migration decline to run — see LegacyDataMigration.
        try { LegacyDataMigration.Run(); } catch { }
        ...
        try
        {
            ...
            WriteHeader();
            HookGlobalHandlers();
            if (LegacyDataMigration.Report is { Length: > 0 } report) Write(report);
        }
