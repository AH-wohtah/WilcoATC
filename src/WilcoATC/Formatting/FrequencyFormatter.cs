...
public static class FrequencyFormatter
{
    ...
    /// on laisse le moteur TTS lire le nombre naturellement. Les zéros finaux non
    /// significatifs sont retirés ; la précision 8.33 kHz est conservée.
    /// Ex. 118.700 → « 118.7 » (lu « one eighteen point seven »).
    /// </summary>
    public static string Speak(double hz)
    {
        double mhz = hz / 1_000_000.0;
        mhz = Math.Round(mhz, 3, MidpointRounding.AwayFromZero);
        return mhz.ToString("0.###", CultureInfo.InvariantCulture); // 118.700 -> "118.7"
    }
}
