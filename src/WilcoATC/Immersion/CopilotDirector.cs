...
/// <summary>
/// Vitesses de référence + option checklists (réglages), et le GABARIT de l'appareil :
/// un avion léger n'a ni V1, ni spoilers, ni inverseurs, et souvent pas de train rentrant.
/// Annoncer « V one » dans un Cessna casse l'immersion aussi sûrement qu'un silence.
/// </summary>
public readonly record struct CopilotConfig(
    int V1, int Vr, int V2, bool Checklists,
    bool Light = false, bool GearRetractable = true);
...
public sealed class CopilotDirector
{
    ...
    public IReadOnlyList<string> Update(CopilotState s, CopilotConfig cfg)
    {
        ...
        // Le « quatre-vingts nœuds » est un repère de ligne : sur un avion léger qui décolle
        // vers 55 kt il tomberait après la rotation. On annonce « airspeed alive » à la place.
        if (s.OnGround && cfg.Light && Crossed(p.IasKnots, s.IasKnots, LightAirspeedAliveKnots, up: true))
            Say(say, "airspeed_alive");
        if (s.OnGround && !cfg.Light && Crossed(p.IasKnots, s.IasKnots, 80, up: true)) Say(say, "eighty");
        ...
        {
            // « Train rentré » n'a de sens que sur un train rentrant : sur un Cessna à train
            // fixe, l'annonce désigne une action que le pilote ne peut pas faire.
            if (Say(say, "positive_rate") && cfg.GearRetractable) Say(say, "gear_up");
        }
        ...
        if (!s.OnGround && s.VerticalSpeedFpm < -200)
        {
            ...
            // « Minimums » se rapporte à une hauteur de décision d'approche aux instruments.
            // En vol à vue il n'y en a pas : on annonce la hauteur, sans le mot.
            if (Crossed(p.AglFeet, s.AglFeet, 200, up: false))
                Say(say, cfg.Light ? "app_200" : "minimums");
        ...
        // Spoilers, inverseurs et « soixante-dix nœuds » sont des repères d'avion de ligne.
        if (s.OnGround && !p.OnGround && s.GroundSpeedKnots > 40 && !cfg.Light)
        {
            Say(say, "spoilers");
            Say(say, "reverse");
        }
    ...
    /// <summary>Vitesse à laquelle l'anémomètre devient exploitable sur un avion léger.</summary>
    private const double LightAirspeedAliveKnots = 40;
