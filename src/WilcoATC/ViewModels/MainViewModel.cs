...
public sealed class MainViewModel : ObservableObject
{
    ...
    // Sélecteur de phase forcée (DEBUG) : "Auto" + phases de la machine à états.
    public IReadOnlyList<string> PhaseOverrides { get; } = new[]
        { "Auto", "Parked", "Pushback", "TaxiOut", "Takeoff", "Airborne", "Approach", "Landing", "TaxiIn" };

    private string _selectedPhaseOverride = "Auto";
    public string SelectedPhaseOverride
    {
        get => _selectedPhaseOverride;
        set
        {
            if (!SetProperty(ref _selectedPhaseOverride, value)) return;
            FlightPhase? ov = value == "Auto" || !Enum.TryParse<FlightPhase>(value, out var p) ? null : p;
            _atc.SetPhaseOverride(ov);
        }
    }

    private bool _forceHasBeenAirborne;
    /// <summary>Force le flag « déjà été en l'air » (coché = true, décoché = auto).</summary>
    public bool ForceHasBeenAirborne
    {
        get => _forceHasBeenAirborne;
        set { if (SetProperty(ref _forceHasBeenAirborne, value)) _atc.SetHasBeenAirborneOverride(value ? true : null); }
    }
    ...
    private async Task ToggleListenAsync()
    {
        ...
        // 2ᵉ appui : on ferme le micro, on transcrit, et on injecte le texte comme une requête.
        IsListening = false;
        ...
        : this(new NullSimConnectService(), new NullStationResolver(), new NullAtcController(), new NullSpeechToText(), new FlightPlanStore(), Dispatcher.CurrentDispatcher)
    {
        ...
        AddSystemLog("Connecté au simulateur.");
    }
}
